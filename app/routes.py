...
def team_details(group_id):
    ...
    db.session.expire(grupo, ['produtos'])
...
@bp.route('/consultor/visualizar_lead/<int:lead_id>')
@login_required
@require_role('consultor')
def visualizar_lead(lead_id):
    ...
    # Preparar os dados do lead como na função atendimento
    cleaned_telefone = re.sub(r'\D', '', lead.telefone) if lead.telefone else None
    cleaned_telefone_2 = re.sub(r'\D', '', lead.telefone_2) if lead.telefone_2 else None
    ...
    return render_template('visualizar_lead.html',
